...
# Build bounding box CSV file.
mkdir -p "${BBOX_DIR}"
tar xzf "${BBOX_TAR_BALL}" -C "${BBOX_DIR}"
"${BOUNDING_BOX_SCRIPT}" "${BBOX_DIR}/" "${SYNSETS_FILE}" | sort > "${BOUNDING_BOX_FILE}"
[ -f "${BOUNDING_BOX_FILE}" ] || { echo "File ${BOUNDING_BOX_FILE} not found" ; exit 1 ;}
