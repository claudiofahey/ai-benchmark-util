...
def add_test():
    ...
    data_format = 'NCHW' if device == 'gpu' else 'NHWC'
    force_gpu_compatible = device == 'gpu'
    ...
    t = dict(
        ...
        command_template=[
            ...
            '--device', device,
            '--flush', '%d' % flush,
            '--force_gpu_compatible', '%d' % force_gpu_compatible,
            '--fp16', '%d' % fp16,
...
for repeat in range(3):
    for storage_type in ['isilon']:
        for cached in [True] if storage_type=='local' else [True]:
